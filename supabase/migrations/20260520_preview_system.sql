...
-- Preview System for Draft Pages

-- 1. Update Pages RLS to allow authenticated users to see all pages (for editor preview)
...
-- 2. Create a SECURITY DEFINER function to fetch a page (including drafts) by ID
-- This bypasses RLS safely because it only returns a single page if the ID is known
...
-- 3. Explicitly grant execute permission
